...
应用查询的正式单词存放在数据库中；Git 同时保存可重建数据库的固定版本 JSONL 数据包。当前数据包合并 CET-4、CET-6、考研通用、考研英语一/英语二和 IELTS，去重后为 7,416 个词条。从 GitHub 克隆后不需要 `qwerty-learner` 目录即可校验和导入：
...
访问：http://localhost 。首次启动会自动迁移数据库、写入示例文章，并从 Git 数据包导入统一英文核心词书。生产环境默认不创建演示账号，请自行注册第一个用户。
