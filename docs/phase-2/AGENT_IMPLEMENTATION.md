...
最后核对日期：2026-09-07。代码行为发生变化时，应同时更新本文和相关自动化测试。
...
- 模型每轮只接收最近 24 条消息、较早对话的简化摘要、用户资料和明确保存的长期偏好，不会把全部历史消息原样发送给厂商。
- 学习记录、复习计划、生词本和文章等属于业务事实，不当作模型记忆，而是需要时从数据库实时查询。
- 所有会话消息都可以按所属会话从数据库恢复，但当前没有跨全部会话的语义搜索、向量库或 RAG。因此“所有历史”如果指任意旧会话中的细节，目前只能逐个打开会话查看，Agent 不会自动检索全部旧会话。
...
- `conversation_id`：继续已有会话时传入；
- `provider`：创建会话时可选，未指定时使用已启用的默认厂商。
...
3. 如果是新会话，选择当前用户已启用的默认 Provider，创建会话并固定厂商与模型；
...
2. 较早对话摘要（存在时）；
3. 当前会话最近消息；
4. 本轮工具循环中新产生、尚未重新从数据库读取的 assistant/tool 消息。

系统提示词包含当前用户的本地日期、IANA 时区、英语等级、每日新词目标和明确保存的长期偏好。它还要求模型对学习事实和数据操作必须调用工具，不得自行猜测。
...
1. 保存完整 assistant 消息、结束原因和用量；
2. 提交事务；
3. 发送 `message.completed`；
4. 发送 `usage.completed`，其中包含厂商、模型、累计 Token 和本轮延迟；
5. 结束 SSE。
...
| `ai_conversations` | 用户、标题、固定 Provider/模型、较早摘要、归档状态 | 删除会话时级联清理 |
| `ai_messages` | user/assistant/tool 内容、Tool Calls、用量、厂商元数据 | 完整持久化 |
...
| 工具 | 类型 | 行为 |
|---|---|---|
| `lookup_word` | 只读 | 查内置词典、掌握度和是否在生词本 |
...
- `message.completed`：把临时消息替换为持久化消息 ID；
- `error`：显示安全化错误信息。

重新打开会话时，前端分别读取消息和工具记录。普通对话区过滤 `role=tool` 消息，工具结果从 `ai_tool_runs` 恢复为卡片。
...
8. 会话标题取首条消息前 24 个字符，尚未做智能标题生成。

优先改进顺序建议：先增加跨会话可控检索与长期记忆管理，再升级摘要；第三阶段处理文件时引入持久化任务表和单机 worker，不应直接把耗时任务塞进当前 SSE 请求。
