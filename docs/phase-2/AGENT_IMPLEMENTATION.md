...
最后核对日期：2026-09-10。代码行为发生变化时，应同时更新本文和相关自动化测试。
...
- 对话历史侧栏在桌面和移动端都默认收起。只读查询工具卡只在当前请求期间显示，最终回答完成后清理；写入、确认和失败结果仍可恢复。
...
系统提示词包含当前用户的本地日期、IANA 时区、每日新词目标和明确保存的长期偏好。CEFR 等级已从 Agent 上下文撤下。提示词要求模型对学习事实和数据操作必须调用工具，不得自行猜测；引用旧对话时调用跨会话检索；查词缺失时先询问，只有用户明确同意后才能创建私有词条。
...
重新打开会话时，前端分别读取消息和工具记录。普通对话区过滤 `role=tool` 消息。写操作、确认等需要保留的工具结果从 `ai_tool_runs` 恢复为卡片；查词、历史、计划、易错词、生词本列表和跨会话检索属于短生命周期状态，在 `message.completed` 后清理，避免长期堆积在回答下方。

历史会话抽屉由用户主动打开，初始状态在桌面和移动端均为关闭。默认收起只改变界面，不删除数据库会话，也不限制 Agent 的受控历史检索。
