...
最后核对：2026-09-16。当前完成度和已知限制见 [项目状态](PROJECT_STATUS.md)，表级细节见 [数据模型](DATA_MODEL.md)。
...
- `words` 保存去重后的词典条目和 `system|custom` 来源，不存放用户进度；`user_custom_words` 决定自定义词对哪些用户可见。
...
- `study_sessions` 保存一次完整学习组；每个词在 `study_reviews` 中保存该组的分数轨迹、错误次数、轮数和响应数据。

## 5. 复习算法

学习会话提供“不记得 0、模糊 50、熟悉 90”三个快捷值，同时允许用 0–100 滑杆记录精细熟悉度。低于 80 分的单词进入下一轮；整组全部通过后才批量写入数据库。当前兼容调度器按完整组内轨迹归类为 `forgot|fuzzy|remembered` 后更新既有进度，不向用户展示单次选择推导的间隔。后续复习算法可直接使用分数序列、错误次数、轮数和响应时间替换该调度器。

未完成学习组按用户和学习来源保存在浏览器 IndexedDB；刷新后可在同一浏览器恢复。服务器以 `user_id + client_session_id` 保证整组提交幂等，并在同一事务中写会话、逐词历史和进度。未完成组当前不跨设备同步。
...
当前会话只向模型发送最近消息和轻量摘要。用户引用旧对话时，Agent 通过受限工具按当前 `user_id` 检索全部会话中的关键词，而不是把完整历史在每轮发送给模型。未指定会话时，后端按用户 IANA 时区的本地日期复用一条 `daily` 会话；用户显式新建的会话标记为 `manual`，两者不会混用。

输入适配层在模型调用前将纯英文输入确定性区分为单词、句子和长文，并注入本轮任务约束；回答完成后再根据输入分类和已验证工具结果生成结构化操作。模型负责正文与工具选择，但不能自行构造可执行 URL 或绕过服务端操作白名单。AI 补充的新词先创建用户所有权映射，任何查词、加词或学习写入都必须满足“系统词或当前用户拥有的自定义词”。
...
Assistant page
  ...
  → 查找/创建当天默认会话（或使用显式手动会话）
  → 持久化 user message
  → 确定性识别普通问题/英文单词/句子/长文
  → 有界 Agent loop
      → 解密当前用户 Provider Key
      → 发送最近上下文 + 工具 schema
      → 校验 Tool Call
  ...
  → 持久化正文、输入类型和白名单 response actions
  → SSE events
  → 前端安全 Markdown 渲染并按白名单执行操作，可从数据库恢复
```
...
- 全局词典与用户学习状态分离。
- 自定义词以显式用户映射控制可见性；仅靠前端隐藏不构成权限控制。
