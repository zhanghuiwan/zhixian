...
export default function ArticleReaderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className={`reader-page reader-workspace ${aiOpen ? "with-ai" : ""}`}>
    ...
    <main className="reader-main"><article className="continuous-article">
      <header><div><span>{article.topic} · {article.read_minutes} 分钟</span>{article.is_private && <small>AI 生成草稿</small>}</div><h1>{article.title}</h1><h2>{article.title_zh}</h2><p>{article.summary}</p></header>
    ...
    {(selectedWord || lookupMessage) && <aside className="dictionary-popover" aria-live="polite"><button className="close-dictionary" onClick={() => { setSelectedWord(null); setLookupMessage(""); }} aria-label="关闭"><X size={18} /></button>{selectedWord ? <><div className="dictionary-title"><div><div className="word-title-row"><h2>{selectedWord.term}</h2><span className={`word-origin ${selectedWord.dictionary_source}`}>{selectedWord.dictionary_source === "custom" ? "我的新增" : "系统词"}</span></div><span>{selectedWord.phonetic}</span></div><button className="sound-button small" onClick={() => speak(selectedWord.term)}><Volume2 size={17} /></button></div><p className="dictionary-meaning"><em>{selectedWord.part_of_speech}</em>{selectedWord.translation}</p>{selectedWord.example && <div className="dictionary-example"><p>{selectedWord.example}</p><span>{selectedWord.example_translation}</span></div>}<button className="primary-button wide" disabled={wordSaved} onClick={() => void saveSelectedWord()}>{wordSaved ? "已加入生词本" : "加入生词本"}</button></> : <div className="lookup-empty"><p>{lookupMessage}</p><button className="secondary-button" onClick={() => { setAiOpen(true); setAiInput(`请先解释单词 ${lookupTerm}。如果系统词库未收录，请询问我是否把它加入我的新增单词。`); }}>让 AI 帮我补充</button></div>}</aside>}
