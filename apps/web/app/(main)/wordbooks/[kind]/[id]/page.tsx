...
export default function WordbookDetailPage({ params }: { params: Promise<{ kind: string; id: string }> }) {
  ...
  return <div className="page workspace-page">
    ...
    <div className="book-word-list">{data.words.map((item) => <article key={item.word.id}><div><h2>{item.word.term}</h2><span>{item.word.phonetic}</span></div><p><em>{item.word.part_of_speech}</em>{item.word.translation}</p><div className="word-state"><span>{item.status === "new" ? "未学习" : item.status === "mastered" ? "已掌握" : `熟练度 ${item.mastery_score}%`}</span>{validKind === "personal" && <button onClick={() => removeWord(item.word.id)} aria-label={`从词书移除 ${item.word.term}`}><Trash2 size={16} /></button>}</div></article>)}</div>
